...
def generate_bria(cop_node: hou.Node) -> None:
    ...
    try:
        ...
        out_path = os.path.join(temp_dir, f"bria_genfill_result_{run_id}.png")
        ...
        if api_key:
            msg = "Bria GenFill: api_token parm is deprecated. Prefer config/env-based auth (BRIA_API_KEY_HOUDINI or ~/.bria/bria.json)."
            _debug_log(msg)
            try:
...
def on_generate(kwargs: dict) -> None:
