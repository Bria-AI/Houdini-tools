...
def _parse_structured_prompt(raw: Optional[str]) -> Optional[dict]:
    if not raw:
        return None
    try:
...
def fibo_generate_bria(cop_node: hou.Node) -> None:
    ...
    try:
        ...
        out_path = os.path.join(temp_dir, f"bria_fibo_generate_result_{run_id}.png")
