"""Houdini Bria RMBG (Remove Background) node logic."""
...
def remove_background_bria(cop_node: hou.Node) -> None:
    ...
    try:
        ...
        out_path = os.path.join(temp_dir, f"bria_rmbg_result_{run_id}.png")
        ...
        if api_key:
            msg = "Bria RMBG: api_token parm is deprecated. Prefer config/env-based auth (BRIA_API_KEY_HOUDINI or ~/.bria/bria.json)."
            _debug_log(msg)
            try:
