...
def restyle_viewport(node) -> Optional[str]:
    ...
    try:
        ...
        # Bria currently requires steps_num >= 20 when explicitly provided.
        steps_val = _eval_parm_int(node, "steps_num") or 0
        if steps_val <= 0:
            steps_num = None
        else:
            steps_num = max(20, int(steps_val))
            if steps_num != int(steps_val):
                logger.info(
                    "steps_num=%s is below minimum; clamping to 20 for viewport restyle request.",
                    steps_val,
                )
