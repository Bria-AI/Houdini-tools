...
- It sets `BRIA_HOUDINI_ROOT` and `HOUDINI_PACKAGE_DIR` so Houdini loads [houdini/packages/bria_houdini.json](houdini/packages/bria_houdini.json).

## 456.py (dev-only)

- [tools/dev/456.py](tools/dev/456.py) is a Houdini startup hook used for testing.
- It only prepares `sys.path`. Bootstrap is now triggered from package-level import in [houdini/__init__.py](houdini/__init__.py).
