"""Houdini Bria GenFill node logic (thin DCC adapter layer)."""
...
from houdini.adapter import genfill_from_files
...
if not hasattr(hou.session, "bria_genfill_session"):
    hou.session.bria_genfill_session = None


_debug_log = debug_logger("Bria GenFill")


def genfill_bria(cop_node: hou.Node) -> None:
    t_start_click = time.perf_counter()

    try:
        prompt = (cop_node.evalParm("prompt") or "").strip()
        if not prompt:
        ...
        img_path = os.path.join(temp_dir, f"bria_genfill_input_{run_id}.png")
        mask_path = os.path.join(temp_dir, f"bria_genfill_mask_{run_id}.png")
        out_path = resolve_output_dir(temp_dir, run_id, "genfill")
        ...
        refine_prompt = _opt_parm_bool(cop_node, "refine_prompt")
        ...
        if api_key:
            msg = "Bria GenFill: api_token parm is deprecated. Prefer config/env-based auth."
            _debug_log(msg)
            try:
        ...
        data = genfill_from_files(
            ...
            tailored_model_id=tailored_model_id if tailored_model_id else None,
            ...
            proxies=proxies,
            session=hou.session.bria_genfill_session,
        )
        ...
        _debug_log(f"API GenFill Time: {api_time:.4f} sec")
        ...
        save_api_metadata(save_path, data, "genfill", {
...
def on_genfill(kwargs: dict) -> None:
