"""Houdini Bria FIBO Edit Presets node logic (thin DCC adapter layer).
...
if not hasattr(hou.session, "bria_fibo_edit_presets_session"):
    hou.session.bria_fibo_edit_presets_session = None


_debug_log = debug_logger("Bria FIBO Edit Presets")
...
def fibo_edit_presets_bria(cop_node: hou.Node) -> None:
    ...
    try:
        ...
        img_path = os.path.join(temp_dir, f"bria_fibo_edit_presets_input_{run_id}.png")
        out_path = resolve_output_dir(temp_dir, run_id, "fibo_edit_presets")
        ...
        data = fibo_edit_from_files(
            ...
            proxies=proxies,
            session=hou.session.bria_fibo_edit_presets_session,
        )
        ...
        save_api_metadata(save_path, data, "fibo_edit_presets", {
        ...
        error_msg = f"Bria FIBO Edit Presets Error: {_safe_exc_str(e)}"
...
def on_fibo_edit_presets(kwargs: dict) -> None:
    node = kwargs.get("node")
    if node is None:
        raise ValueError("Missing kwargs['node']")
    fibo_edit_presets_bria(node)
